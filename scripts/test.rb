...
Orocos.run "video_streamer_vlc::Task" => "task2", :valgrind => false, :valgrind_options => ["--leak-check=full"]  do 
...
#    config.config.bitrate = 10
    ...
    #config.config.raw_config = "#transcode{vcodec=mp2v,scale=0.5,threads=4}:rtp{dst=239.255.12.42,port=5004,ttl=50}" 
    config.config.raw_config = "#transcode{vcodec=mjpg,scale=0.5,vb=100}:rtp{mux=ts,dst=239.255.12.42,port=5004,ttl=50}" 
